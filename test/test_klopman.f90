...
module test_klopman
   ...
   use multicharge_coulomb_klopman, only : mchrg_klopman_coulomb, new_klopman_coulomb, &
      & harmonic_average, arithmetic_average
   implicit none
   private

   public :: collect_klopman
   ...
   abstract interface
      subroutine coulomb_maker(coulomb, mol)
         import :: mchrg_klopman_coulomb, structure_type
         type(mchrg_klopman_coulomb), intent(out) :: coulomb
...
subroutine collect_klopman(testsuite)
...
end subroutine collect_klopman


!> Factory to create electrostatic objects based on GFN1-xTB values
subroutine make_coulomb1(coulomb, mol)
   ...
   hardness = atomic_hardness(mol%num)
   call new_klopman_coulomb(coulomb, gexp, hardness, harmonic_average)

end subroutine make_coulomb1


!> Factory to create electrostatic objects based on GFN2-xTB values
subroutine make_coulomb2(coulomb, mol)
   ...
   hardness = atomic_hardness(mol%num)
   call new_klopman_coulomb(coulomb, gexp, hardness, arithmetic_average)
...
subroutine test_generic(error, mol, charges, make_coulomb, ref)
   ...
   type(mchrg_klopman_coulomb) :: coulomb
...
subroutine test_e_effective_m01(error)
   ...
   call test_generic(error, mol, charges, make_coulomb1, 0.10952019883948200_wp)
...
subroutine test_e_effective_m02(error)
   ...
   call test_generic(error, mol, charges, make_coulomb2, 0.10635843572138280_wp)
...
subroutine test_e_effective_oxacb(error)
   ...
   call test_generic(error, mol, charges, make_coulomb2, 0.39165014434415341_wp)
...
subroutine test_g_effective_m03(error)
   ...
   call test_numgrad(error, mol, charges, make_coulomb1)
...
subroutine test_g_effective_m04(error)
   ...
   call test_numgrad(error, mol, charges, make_coulomb2)
...
subroutine test_s_effective_m05(error)
   ...
   call test_numsigma(error, mol, charges, make_coulomb1)
...
subroutine test_s_effective_m06(error)
   ...
   call test_numsigma(error, mol, charges, make_coulomb2)
